...
/** Kept for compatibility with older response rows that stored contact details. */
export const CONTACT_QUESTION_ID = 's18_contact';
...
/**
 * `last_section` is the 1-based section number most recently completed.
 * On final submit it is TOTAL_SECTIONS.
 */
export function sectionsCompletedFrom(lastSection: number | null): number {
  if (lastSection === null || lastSection === undefined) return 0;
  return Math.max(0, lastSection);
}
